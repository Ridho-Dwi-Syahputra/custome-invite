      ...
      {ps.map(p => (
        <div key={p.id} className="absolute rounded-full"
          style={{
            ...
            background: `radial-gradient(circle, rgba(242,178,74,${p.op}), transparent)`,
...
function CoverPage({ onOpen }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center"
      style={{ background: 'linear-gradient(135deg, #2A160B 0%, #140B06 50%, #0F0804 100%)' }}>
      <Particles />
      <div className="absolute rounded-full pointer-events-none select-none"
        style={{
          ...
          background: 'radial-gradient(circle, rgba(242,178,74,0.12) 0%, transparent 70%)'
        }} aria-hidden="true" />
      <div style={{ width: '88%', maxWidth: 380, margin: '0 auto', position: 'relative', zIndex: 10, animation: 'scaleIn 0.6s ease-out forwards' }}>
        <div className="glass-strong rounded-3xl text-center shadow-2xl"
          ...
          <div className="flex justify-center mb-5 pointer-events-none select-none">
            <img src={logoMetro} alt="Logo BKHIT SUMBAR" style={{ width: 64, height: 64, objectFit: 'contain' }} draggable="false" />
          </div>
          <h1 className="font-serif text-white select-none" style={{ fontSize: 30, fontWeight: 700, marginBottom: 2 }}>Seminar</h1>
          <h2 className="font-serif text-purple-300 select-none" style={{ fontSize: 20, fontWeight: 600, marginBottom: 28 }}>Magang</h2>
          <div style={{ width: 80, height: 1.5, margin: '0 auto 28px', background: 'linear-gradient(90deg, transparent, rgba(242,178,74,0.6), transparent)' }} aria-hidden="true" />
          ...
          <button onClick={onOpen}
            ...
            style={{
              ...
              background: 'linear-gradient(135deg, #C97116, #F2B24A)', color: '#fff',
              fontSize: 16, fontWeight: 600, boxShadow: '0 8px 24px rgba(242,178,74,0.3)',
              transition: 'all 0.3s', position: 'relative', overflow: 'hidden'
            }}
            onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.02)'; e.currentTarget.style.boxShadow = '0 12px 32px rgba(242,178,74,0.45)' }}
            onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(242,178,74,0.3)' }}>
...
/* ── Countdown ── */
function Countdown() {
  const target = new Date('May 21, 2026 13:30:00').getTime()
    ...
    <div style={{ display: 'flex', justifyContent: 'center', gap: 12 }}>
      {items.map((b, i) => (
        <div key={b.l} className="glass select-none"
          ...
          <span style={{ display: 'block', fontSize: 10, color: 'rgba(255,232,200,0.7)', marginTop: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
            {b.l}
          </span>
        </div>
      ))}
    ...
    <>
      <form onSubmit={submit}>
        <div style={{ marginBottom: 16 }}>
          <label className="select-none" style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'rgba(255,232,200,0.7)', marginBottom: 8 }}>Pengirim</label>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Masukkan nama Kamu..." required
            style={inputStyle} onFocus={e => e.target.style.borderColor = '#C97116'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
        </div>
        <div style={{ marginBottom: 16 }}>
          <label className="select-none" style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'rgba(255,232,200,0.7)', marginBottom: 8 }}>Pesan Ucapan</label>
          <textarea value={msg} onChange={e => setMsg(e.target.value)} placeholder="Tuliskan ucapan & doa terbaik Kamu..." required rows="3"
            style={{ ...inputStyle, resize: 'none' }} onFocus={e => e.target.style.borderColor = '#C97116'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
        </div>
        <button type="submit" disabled={busy} className="cursor-pointer select-none"
          style={{
            ...
            boxShadow: '0 6px 20px rgba(242,178,74,0.25)', transition: 'all 0.3s'
          }}>
      ...
      <div style={{ marginTop: 24, maxHeight: 340, overflowY: 'auto' }}>
        {loading ? (
          <p className="select-none" style={{ textAlign: 'center', color: 'rgba(255,232,200,0.35)', padding: '24px 0', fontSize: 13 }}>Memuat ucapan...</p>
        ) : list.length === 0 ? (
          <p className="select-none" style={{ textAlign: 'center', color: 'rgba(255,232,200,0.35)', padding: '24px 0', fontSize: 13 }}>Belum ada ucapan. Jadilah yang pertama! </p>
        ) : list.map(c => (
          <div key={c.id || c.timestamp} className="glass animate-fade-in" style={{ borderRadius: 12, padding: 16, marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <span className="select-none" style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,232,200,0.9)' }}>{c.name}</span>
              <span className="select-none" style={{ fontSize: 10, color: 'rgba(242,178,74,0.55)' }}>{fmt(c.timestamp)}</span>
            </div>
...
export default function App() {
  ...
  return (
    <>
      ...
      {isOpen && !show && (
        <div style={{
          position: 'fixed', inset: 0, zIndex: 50, background: '#140B06',
          animation: 'fadeIn 0.4s ease-out reverse forwards'
        }} />
      )}

      {show && (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #2A160B 0%, #140B06 50%, #0F0804 100%)', position: 'relative' }}>
          <Particles />
          ...
          <div className="pointer-events-none select-none" style={{
            ...
            background: 'radial-gradient(circle, rgba(242,178,74,0.18), transparent 70%)'
          }} aria-hidden="true" />
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <div style={frameStyle}>
              ...
              <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: 2, background: 'linear-gradient(to bottom, transparent, rgba(242,178,74,0.12), transparent)' }} aria-hidden="true" />
              <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: 2, background: 'linear-gradient(to bottom, transparent, rgba(242,178,74,0.12), transparent)' }} aria-hidden="true" />
              ...
              <R delay={100}>
                <div style={{ textAlign: 'center', marginBottom: 48 }}>
                  ...
                  <div style={{ width: 72, height: 1.5, margin: '20px auto 0', background: 'linear-gradient(90deg, transparent, rgba(242,178,74,0.6), transparent)' }} aria-hidden="true" />
                </div>
              </R>
              ...
              <R delay={150}>
                <div style={{ textAlign: 'center', marginBottom: 48 }}>
                  <div className="pointer-events-none select-none" style={{
                    ...
                    <div style={{ position: 'absolute', inset: 3, borderRadius: '50%', background: 'linear-gradient(135deg, #F2B24A, #A55A12)' }} aria-hidden="true" />
                    <div style={{ position: 'absolute', inset: 6, borderRadius: '50%', overflow: 'hidden' }}>
                      <img src={fotoRidho} alt="Foto Sultan" style={{ width: '100%', height: '100%', objectFit: 'cover' }} draggable="false" />
                    </div>
                  </div>
                  <h2 className="font-serif text-white select-none" style={{ fontSize: 26, fontWeight: 700, marginBottom: 4 }}>Sultan Chairuhman Yahya</h2>
                  <p className="select-none" style={{ fontSize: 13, color: 'rgba(255,232,200,0.6)' }}>Mahasiswa sosial ekonomi pertanian</p>
                </div>
              </R>
              ...
              <R delay={100}>
                <div className="glass-strong" style={{ borderRadius: 20, padding: '28px 24px', textAlign: 'center', marginBottom: 48 }}>
                  <span className="select-none pointer-events-none" style={{
                    ...
                    background: 'rgba(242,178,74,0.15)', color: 'rgba(255,232,200,0.85)',
                  ...
                  <h3 className="font-serif text-white select-none" style={{ fontSize: 14, lineHeight: 1.8, fontWeight: 600 }}>
                    PERAN PHYTOSANITARY CERTIRICATE DALAM MENDUKUNG EKSPOR KOMODITI PERTANIAN SUMATERA BARAT DI BKHIT SUMBAR
                  </h3>
                </div>
              </R>
              ...
              <R delay={100}>
                <div style={{ textAlign: 'center', marginBottom: 48 }}>
                  <p className="font-serif select-none" style={{ fontSize: 18, fontWeight: 600, color: 'rgba(255,232,200,0.85)', fontStyle: 'italic', marginBottom: 20 }}>
                    Assalamu'alaikum Warahmatullahi Wabarakatuh,
                  </p>
                  <p className="select-none" style={{ fontSize: 14, lineHeight: 1.85, color: 'rgba(255,255,255,0.7)' }}>
                    Melalui undangan ini, aku mengundang dan meminta dukungan Abang/Kakak dan teman-teman sekalian pada Seminar Magang aku, yang insyaallah akan diselenggarakan pada:
                  </p>
                </div>
              </R>
              ...
              <R delay={100}>
                <div className="glass" style={{ borderRadius: 20, padding: '24px 24px', marginBottom: 48 }}>
                  {[
                    { icon: <><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></>, label: 'Hari & Tanggal', value: 'Kamis, 21 Mei 2026' },
                    { icon: <><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></>, label: 'Waktu', value: '13.30 - 14.30' },
                    { icon: <><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></>, label: 'Tempat', value: 'Ruang Seminar departemen sosek' },
                  ].map((item, i) => (
                    <div key={i}>
                      ...
                      <div className="select-none cursor-default" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div className="pointer-events-none" style={{
                          width: 40, height: 40, borderRadius: '50%', background: 'rgba(242,178,74,0.15)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0
                        }}>
                          <svg width="18" height="18" fill="none" stroke="rgba(255,232,200,0.85)" viewBox="0 0 24 24" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                            {item.icon}
                          </svg>
                        </div>
                        <div>
                          <p style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,232,200,0.65)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 2 }}>{item.label}</p>
              ...
              <R delay={100}>
                <div style={{ textAlign: 'center', marginBottom: 48 }}>
                  <h3 className="font-serif select-none" style={{ fontSize: 17, fontWeight: 600, color: 'rgba(255,232,200,0.7)', marginBottom: 20 }}>
                    Menuju Hari H
                  </h3>
                  <Countdown />
                </div>
              ...
              <div className="pointer-events-none select-none" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, margin: '0 0 48px' }} aria-hidden="true">
                <div style={{ width: 60, height: 1, background: 'linear-gradient(90deg, transparent, rgba(242,178,74,0.25))' }} />
                <div style={{ width: 5, height: 5, borderRadius: '50%', background: 'rgba(242,178,74,0.3)' }} />
                <div style={{ width: 60, height: 1, background: 'linear-gradient(90deg, rgba(242,178,74,0.25), transparent)' }} />
              </div>
              ...
              <R delay={100}>
                <div style={{ textAlign: 'center', marginBottom: 16 }}>
                  <span className="select-none pointer-events-none" style={{
                    display: 'inline-block', padding: '4px 14px', borderRadius: 20,
                    background: 'rgba(242,178,74,0.12)', color: 'rgba(255,232,200,0.7)',
                    fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: 16
                  }}>Tempat Magang</span>
                  <h4 className="text-white select-none" style={{ fontSize: 15, fontWeight: 600, marginBottom: 2 }}>BKHIT SUMBAR</h4>
                  <p className="select-none" style={{ fontSize: 11, color: 'rgba(255,232,200,0.45)' }}>Instansi</p>
                </div>
              </R>
              ...
              <R delay={100}>
                <div style={{ textAlign: 'center', marginBottom: 48 }}>
                  ...
                  <p className="font-serif select-none" style={{ fontSize: 18, fontWeight: 600, color: 'rgba(255,232,200,0.85)', fontStyle: 'italic', marginBottom: 32 }}>
                    Wassalamu'alaikum Warahmatullahi Wabarakatuh.
                  </p>
                  <p className="select-none" style={{ fontSize: 12, color: 'rgba(255,232,200,0.5)', marginBottom: 6 }}>Hormat Saya,</p>
                  <p className="font-serif text-white select-none" style={{ fontSize: 20, fontWeight: 700 }}>Sultan Chairuhman Yahya</p>
                </div>
              </R>
              ...
              <R>
                <div style={{ textAlign: 'center', paddingBottom: 16 }}>
                  <Ornament flip />
                  <p className="select-none" style={{ fontSize: 10, color: 'rgba(242,178,74,0.25)', marginTop: 20 }}></p>
                </div>
              </R>
